...
import random
from mpo_model import Prayer, Category, Panel, StateTransitionTable, AppParms
...
class App:
    ...
    def __init__(self):
        ...
        assert check_state_transition_table(
        ...
        if yesterday == datetime.strptime(
                AppParms.app_parms.last_prayer_date, '%d-%b-%Y'):
            AppParms.app_parms.prayer_streak += 1
        else:
            AppParms.app_parms.prayer_streak = 1
        AppParms.app_parms.last_prayer_date = \
            current_date.strftime('%d-%b-%Y')
        pass
        # datetime.strptime(current_date, '%d-%b-%Y')
            ...
            for sobj in StateTransitionTable.all_rows:
                if sobj.from_state == panel_header:
                    match sobj.action_event:
                        case 'get_continue':
                            self.uim.get_response('enter to continue ')
                            # special commands from the continue prompt
                            match self.uim.command:
                                case 'import':
                                    pass
                                    # do not import because deleting
                                    #   the pickle files causes import
                                    # self.dbm.import_panels()
                                    # self.dbm.import_prayers()
                                case 'export':
                        ...
                        case _:
                            assert False, f'unexpected s.action_event value: {sobj.action_event}'
    ...
    def get_past_prayers(self):
        ...
        # for n in unique_selected_prayers:
        #     print(n, '| ', n.category, '| ', n.prayer)

        # send the past prayers to the ui manager for display
        display_num = AppParms.app_parms.past_prayer_display_count
        counter = 0
